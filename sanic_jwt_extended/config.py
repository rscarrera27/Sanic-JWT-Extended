...
class Config(object):
    ...
    def header_name(self):
        name = self.app.config['JWT_HEADER_NAME']
        if not name:
    ...
    def header_type(self):
        return self.app.config['JWT_HEADER_TYPE']

    @property
    def query_string_name(self):
        return self.app.config['JWT_QUERY_STRING_NAME']

    @property
    def access_cookie_name(self):
        return self.app.config['JWT_ACCESS_COOKIE_NAME']

    @property
    def refresh_cookie_name(self):
        return self.app.config['JWT_REFRESH_COOKIE_NAME']

    @property
    def access_cookie_path(self):
        return self.app.config['JWT_ACCESS_COOKIE_PATH']

    @property
    def refresh_cookie_path(self):
        return self.app.config['JWT_REFRESH_COOKIE_PATH']

    @property
    def cookie_secure(self):
        return self.app.config['JWT_COOKIE_SECURE']

    @property
    def cookie_domain(self):
        return self.app.config['JWT_COOKIE_DOMAIN']

    @property
    def session_cookie(self):
        return self.app.config['JWT_SESSION_COOKIE']

    @property
    def cookie_samesite(self):
        return self.app.config['JWT_COOKIE_SAMESITE']

    @property
    def json_key(self):
        return self.app.config['JWT_JSON_KEY']

    @property
    def refresh_json_key(self):
        return self.app.config['JWT_REFRESH_JSON_KEY']

    @property
    def csrf_protect(self):
        return self.jwt_in_cookies and self.app.config['JWT_COOKIE_CSRF_PROTECT']

    @property
    def csrf_request_methods(self):
        return self.app.config['JWT_CSRF_METHODS']

    @property
    def csrf_in_cookies(self):
        return self.app.config['JWT_CSRF_IN_COOKIES']

    @property
    def access_csrf_cookie_name(self):
        return self.app.config['JWT_ACCESS_CSRF_COOKIE_NAME']

    @property
    def refresh_csrf_cookie_name(self):
        return self.app.config['JWT_REFRESH_CSRF_COOKIE_NAME']

    @property
    def access_csrf_cookie_path(self):
        return self.app.config['JWT_ACCESS_CSRF_COOKIE_PATH']

    @property
    def refresh_csrf_cookie_path(self):
        return self.app.config['JWT_REFRESH_CSRF_COOKIE_PATH']
    ...
    def _get_depreciated_csrf_header_name(self):
        ...
        old_name = self.app.config.get('JWT_CSRF_HEADER_NAME', None)
        if old_name:
            msg = (
    ...
    def access_csrf_header_name(self):
        return self._get_depreciated_csrf_header_name() or \
               self.app.config['JWT_ACCESS_CSRF_HEADER_NAME']

    @property
    def refresh_csrf_header_name(self):
        return self._get_depreciated_csrf_header_name() or \
               self.app.config['JWT_REFRESH_CSRF_HEADER_NAME']

    @property
    def access_expires(self):
        delta = self.app.config['JWT_ACCESS_TOKEN_EXPIRES']
    ...
    def refresh_expires(self):
        delta = self.app.config['JWT_REFRESH_TOKEN_EXPIRES']
        if not isinstance(delta, datetime.timedelta) and delta is not False:
    ...
    @property
    def algorithm(self):
        return self.app.config['JWT_ALGORITHM']

    @property
    def blacklist_enabled(self):
        return self.app.config['JWT_BLACKLIST_ENABLED']

    @property
    def blacklist_checks(self):
        check_type = self.app.config['JWT_BLACKLIST_TOKEN_CHECKS']
    ...
    def _secret_key(self):
        key = self.app.config['JWT_SECRET_KEY']
        if not key:
            key = self.app.config.get('SECRET_KEY', None)
            if not key:
    ...
    def _public_key(self):
        key = self.app.config['JWT_PUBLIC_KEY']
        if not key:
    ...
    def _private_key(self):
        key = self.app.config['JWT_PRIVATE_KEY']
        if not key:
    ...
    def identity_claim_key(self):
        return self.app.config['JWT_IDENTITY_CLAIM']

    @property
    def user_claims_key(self):
        return self.app.config['JWT_USER_CLAIMS']

    @property
    def user_claims_in_refresh_token(self):
        return self.app.config['JWT_CLAIMS_IN_REFRESH_TOKEN']
    ...
    def error_msg_key(self):
        return self.app.config['JWT_ERROR_MESSAGE_KEY']
