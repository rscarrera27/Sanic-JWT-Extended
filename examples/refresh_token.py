...
# Protect a view with jwt_refresh_token_required, which requires a valid refresh token
# in the request to make new access token.
    ...
    return json(
        dict(
            access_token=await create_access_token(
                identity=token.jwt_identity, app=request.app
            )
        ),
        status=200,
    )
